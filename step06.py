...
def draw():
    ...
    # Hole
    pygame.draw.rect(screen, bg_color, (hole_x, hole_y, hole_width, 50))

    # Blocks
    for idx, (bx, by) in enumerate(block_list):
        pygame.draw.rect(screen, BROWN, (bx, by, block_width, block_height))
        if block_hit[idx]:
            pygame.draw.circle(screen, YELLOW, (bx + block_width // 2, by - 20), 10)
...
while running:
    ...
    # Hole collision
    if (hole_x < player_x + player_width // 2 < hole_x + hole_width) and (player_y + player_height >= ground_y):
